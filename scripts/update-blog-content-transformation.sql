...
-- Update existing blog posts to focus on African women themes
UPDATE blog_posts SET 
    title = CASE 
        WHEN id = 1 THEN 'Empowering African Women in Leadership'
        WHEN id = 2 THEN 'Traditional African Fashion Meets Modern Style'
        WHEN id = 3 THEN 'Building Successful Businesses in Africa'
        ELSE title
    END,
    excerpt = CASE 
        WHEN id = 1 THEN 'Celebrating the remarkable achievements of African women who are breaking barriers and leading change across various industries and communities.'
        WHEN id = 2 THEN 'Exploring how contemporary African designers are revolutionizing fashion by blending traditional elements with modern aesthetics.'
        WHEN id = 3 THEN 'Stories of African women entrepreneurs who are creating innovative solutions and building thriving businesses across the continent.'
        ELSE excerpt
    END,
    author = CASE 
        WHEN id = 1 THEN 'Amara Okafor'
        WHEN id = 2 THEN 'Kemi Adebayo'
        WHEN id = 3 THEN 'Fatima Hassan'
        ELSE author
    END,
    category = CASE 
        WHEN id = 1 THEN 'Leadership'
        WHEN id = 2 THEN 'Fashion & Culture'
        WHEN id = 3 THEN 'Entrepreneurship'
        ELSE category
    END,
    tags = CASE 
        WHEN id = 1 THEN ARRAY['Leadership', 'Empowerment', 'Success']
        WHEN id = 2 THEN ARRAY['Fashion', 'Culture', 'Design']
        WHEN id = 3 THEN ARRAY['Business', 'Entrepreneurship', 'Innovation']
        ELSE tags
    END
WHERE id IN (1, 2, 3);
